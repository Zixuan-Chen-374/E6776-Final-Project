...
class LFUAgent(ReflexAgent):
    ...
    def choose_action(self, observation):
        ...
        if min_idx < 0 or min_idx >= self.n_actions:  # 使用正确的变量名和条件
...
class FIFOAgent(ReflexAgent):
    def __init__(self, n_actions):
        self.n_actions = n_actions
        self.enqueue_order = list(range(n_actions))
    
    def choose_action(self, observation):
        action = self.enqueue_order.pop(0)
        self.enqueue_order.append(action)

        if action < 0 or action >= self.n_actions:
            raise ValueError(f"FIFOAgent: Error index {action}")
        return action
